...
(function() {
  ...
  var getArticle = function(url) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('GET', config.apiUrl + '/articles/sources?handle=' + url, true);
    xhr.responseType = 'json';
    xhr.onload = function() {
      console.log(this);
      if (this.status === 200) {
  ...
  var getPdfHash = function(url, callback) {
  ...
  var checkBySha = function(hash, callback) {
    var xhr = new XMLHttpRequest();
  ...
  chrome.webRequest.onCompleted.addListener(
    function(details) {
      //console.log('asda');
      //console.log(tabToArticle[details.tabId]);
